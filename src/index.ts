 ...
 * @param driverInstance - Neo4j driver instance
 */
export function initializeDriver(driverInstance: Driver): void {
 ...
 */
export async function read<T = Record<string, any>>(
  cypher: string,
 ...
 * @returns Array of result objects of type T
 */
export async function write<T = Record<string, any>>(
  cypher: string,
  params: Record<string, any> = {}
): Promise<T[]> {
  const session = driver.session();
 ...
 */
export async function readSingle<T = Record<string, any>>(
  cypher: string,
  params: Record<string, any> = {}
): Promise<T | null> {
 ...
 * @returns The first result object of type T or null if no results
 */
export async function writeSingle<T = Record<string, any>>(
  cypher: string,
  params: Record<string, any> = {}
): Promise<T | null> {
...
export async function createNode<
  T = Record<string, any>,
  P = Record<string, any>
>(label: string, properties: P, returnAlias = 'n'): Promise<T | null> {
  const cypher = `
...
export async function findNodes<
  T = Record<string, any>,
  P = Record<string, any>
>(label: string, properties?: Partial<P>, returnAlias = 'n'): Promise<T[]> {
 ...
 */
export async function batch<T = Record<string, any>>(
  operations: Array<{
    query: string;
    params?: Record<string, any>;
    isWrite?: boolean;
